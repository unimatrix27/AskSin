...
void Relay::config(void Init(), void Switch(uint8_t), uint8_t minDelay, uint8_t randomDelay) {
	...
	msgRecvCnt = 1;																// unimatrix27: init Counter with 1, because otherwise first button press from button on same device will be missed.
																				// comment unimatrix27: Alternative: introduce flag "never switched" that is reset after first switch
}
...
void Relay::trigger40(uint8_t lngIn, uint8_t cnt) {
	...
	if ((lngIn == 0) && (cnt == msgRecvCnt)){
		Serial << "abbruch wegen cnt = " << cnt << "\r\n";
